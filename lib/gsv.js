...
// http://cbk0.google.com/cbk?output=tile&panoid=FkuG39Rhy1Zbicc1BkkyPg&zoom=3&x=5&y=2
GSV.tile = function(panoid, x, y, zoom) {
  // http://cbk3.google.com/cbk?output=xml&ll=25.034011,121.565371
  //TODO: 
  var args = {
    'output': 'tile',
    'zoom': zoom,
  ...
  var url = 'cbk'+Math.floor(Math.random()*4)+".google.com/cbk";
  if (typeof callback == 'undefined')
    callback = GSV.onSaveTile;

  if (typeof zoom == 'undefined')
    zoom = 4;

  hash = 'tile-'+panoid+"_"+x+"_"+y+"_"+zoom;
  return makeRequest(url, args, false, hash);
};
...
var makeRequest = function(url, args, secure, callback) {
  ...
  if (typeof callback === 'function') {
    ...
    distfile = callback;
    if (GSV.config('dist')) distfile = GSV.config('dist') + "/" + callback;
    //TODO: async version?
    request(options).pipe(fs.createWriteStream(distfile));
  } else {
